...
def extraction(page_url):
    ...
    if req.ok:
        ...
        prod_inf = soup.find(
            'table', {'class': 'table table-striped'}).find_all('td')
        ...
        book_review_scraper['title'] = soup.find(
            'ul', {'class': 'breadcrumb'}).find_all('li')[3].text
        book_review_scraper['price_including_tax'] = prod_inf[3].text.replace(
            "Â", "")
        book_review_scraper['price_excluding_tax'] = prod_inf[2].text.replace(
            "Â", "")
        ...
        if soup.find('div', {'id': 'product_description'}) is None:
            ...
            book_review_scraper['product_description'] = soup.find(
                'div', {'id': 'product_description'}).findNext('p').text
        ...
        if soup.find('ul', {'class': 'breadcrumb'}).find_all('li')[2].text.strip('\n') is None:
            ...
            book_review_scraper['category'] = soup.find(
                'ul', {'class': 'breadcrumb'}).find_all('li')[2].text.strip('\n')
            # default value of star-rating
            book_review_scraper['review_star_rating'] = 0
        product_book = soup.find(
            'p', {'class': 'instock availability'}).find_all('p')
        ...
        book_review_scraper['image_url'] = urljoin(
            req.url, soup.find_all('img')[0]['src'])
...
def write_file_to_jpg_csv(books_info):
    ...
    for book_info in books_info:
        download_image_product(
            book_info['image_url'], (book_info['universal_product_code']), book_info['category'])
